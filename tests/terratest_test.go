...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-fc-network-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Fibre-Channel Network Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/FcNetworkPolicies/%s", moid), expectedJSONTemplate, vars)
}
